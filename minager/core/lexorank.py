...
class Lexorank:
    ...
    def get_lexorank_in_between(
        ...
        if middle_rank_parts == previous_rank_parts:
            middle_rank_parts.append(
                (cls.char_to_int(cls.last_symbol) - cls.char_to_int(cls.first_symbol)) // 2
            )
    ...
    def get_rank_length(cls, objects_count: int) -> int:
        if objects_count == 0:
            ...
            length_required_to_place_all_objects = math.ceil(math.log(objects_count, cls.base))
        return min(
            cls.max_rank_length,
    ...
    def increment_rank(cls, rank: str, objects_count: int) -> str:
        ...
        rank_parts = cls.parse_rank(rank)
