        ...
        if node_id == session.current_node:
            if not session.queue and session.bad_repetition_queue:
                session_update_data['current_node'] = (
                    session.bad_repetition_queue[0] if len(session.bad_repetition_queue) > 0 else None
                )
                ...
                session_update_data['current_node'] = session.queue[0] if len(session.queue) > 0 else None
