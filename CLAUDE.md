...
6. **Business-layer classes may be a plain `Service` (multiple methods on one class per entity/aggregate) or an extracted `UseCase` (single-purpose class for a complex, multi-step operation).** In practice, `Service` classes live in `services.py` and `UseCase` classes live in a separate `use_cases.py` (see `user/services.py` + `user/use_cases.py`, and `learning_session/services.py` + `learning_session/use_cases.py`) — both still count as "Business layer," regardless of which file a given piece of logic lives in. There is currently no fixed rule for *when* to extract a UseCase instead of adding a method to the Service class — this heuristic is still being defined; don't invent one, ask if it's unclear. (So far, extraction has followed one recurring shape: multi-step orchestration that also crosses into another service's Port — e.g. `SignUpUseCase`, and `learning_session`'s `StartSessionUseCase`/`RegenerateQueueUseCase`/`PerformRepetitionUseCase` — but this hasn't been confirmed as *the* rule.)
...
**Known debt:** `KnowledgeTreeClient` (`minager/core/clients/knowledge_tree/client.py`, concrete class `KnowledgeTreeClientImpl`) predates this decision — it subclasses `KnowledgeTreeNodeManager` directly (inheritance, not composition), reaches Persistence, not Business, *and* lives in `core` instead of in `node`. Its Port (`AbstractKnowledgeTreeClient` in `base.py`) has been cleaned up to at least declare the real method signatures (`create`/`get`/`update`/`get_subtree_ids`) — it previously didn't match what was actually called. There is deliberately no DTO crossing this boundary (`get`/`create`/`update` return `models.Node` directly) — a DTO would be aspirational until the Adapter actually calls node's Business layer instead of its Persistence layer, so one wasn't added. Fix the deeper issue (Persistence-layer reach, location in `core`) as part of `node`'s SurORM/layered-architecture migration, not as a standalone task.
